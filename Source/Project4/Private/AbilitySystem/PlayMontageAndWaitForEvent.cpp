...
UP4AbilitySystemComponent* UPlayMontageAndWaitForEvent::GetTargetASC()
{
	return Cast<UP4AbilitySystemComponent>(AbilitySystemComponent);
}
...
void UPlayMontageAndWaitForEvent::Activate()
{
	...
	UP4AbilitySystemComponent* P4ASC = GetTargetASC();
	...
	if (P4ASC)
	{
		...
		if (AnimInstance != nullptr)
		{
			...
			EventHandle = P4ASC->AddGameplayEventTagContainerDelegate(EventTags, FGameplayEventTagMulticastDelegate::FDelegate::CreateUObject(this, &UPlayMontageAndWaitForEvent::OnGameplayEvent));

			if (P4ASC->PlayMontage(Ability, Ability->GetCurrentActivationInfo(), MontageToPlay, Rate, StartSection) > 0.f)
			{
...
void UPlayMontageAndWaitForEvent::OnDestroy(bool AbilityEnded)
{
	...
	if (P4ASC)
	{
		P4ASC->RemoveGameplayEventTagContainerDelegate(EventTags, EventHandle);
	}
