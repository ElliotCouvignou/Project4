...
//      It would be unwise to use this for game logic, and is mostly there to allow you to describe stat clamping.*/
  void UPlayerAttributeSet::PreAttributeBaseChange(const FGameplayAttribute& Attribute, float& NewValue) const
  {
	...
	if (Attribute.AttributeName == GetRageAttribute().AttributeName) {
		NewValue = FMath::Clamp(NewValue, 0.0f, RageMax.GetCurrentValue());
	}
  }
...
void UPlayerAttributeSet::GetLifetimeReplicatedProps(TArray<FLifetimeProperty>& OutLifetimeProps) const
{
	...
	DOREPLIFETIME_CONDITION_NOTIFY(UPlayerAttributeSet, Rage, COND_OwnerOnly, REPNOTIFY_Always);
	DOREPLIFETIME_CONDITION_NOTIFY(UPlayerAttributeSet, RageMax, COND_OwnerOnly, REPNOTIFY_Always);
	DOREPLIFETIME_CONDITION_NOTIFY(UPlayerAttributeSet, RageRegen, COND_OwnerOnly, REPNOTIFY_Always);
